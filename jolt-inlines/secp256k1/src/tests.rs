...
mod mont_equiv_tests {
    ...
    #[test]
    fn mont_secp256k1_fq_matches_arkworks_for_basic_ops() {
        type AFq = ark_secp256k1::Fq;

        let m = &constants::SECP256K1_FQ_MODULUS;
        ...
        let a_m = mont::Secp256k1Fq::from_u64_arr(&a_can).unwrap();
        let b_m = mont::Secp256k1Fq::from_u64_arr(&b_can).unwrap();
        let c_m = mont::Secp256k1Fq::from_u64_arr(&c_can).unwrap();
        let d_m = mont::Secp256k1Fq::from_u64_arr(&d_can).unwrap();
    ...
    #[test]
    fn mont_secp256k1_point_ops_match_arkworks() {
        let g_m = mont::Secp256k1Point::generator();
        let g_a = ark_secp256k1::Projective::generator();
