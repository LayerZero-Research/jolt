...
impl BytecodePreprocessing {
    ...
    pub fn get_unexpanded_pc_vec<F: JoltField>(&self) -> Vec<F> {
        self.bytecode
            .par_iter()
            .map(|instruction| {
                ...
                F::from_u64(unexpanded_pc as u64)
            })
            .collect()
    }

    pub fn get_imm_vec(&self) -> Vec<i128> {
        self.bytecode
            .par_iter()
            .map(|instruction| {
                let NormalizedInstruction {
    ...
    pub fn get_rd_vec(&self) -> Vec<u64> {
        self.bytecode
            .par_iter()
            .map(|instruction| {
                let NormalizedInstruction {
    ...
    pub fn get_circuit_flags_vec(&self, index: usize) -> Vec<u8> {
        self.bytecode
            .par_iter()
            .map(|instruction| {
                instruction.circuit_flags()[index] as u8
            })
            .collect()
    }
