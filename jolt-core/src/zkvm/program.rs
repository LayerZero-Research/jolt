...
impl<PCS: CommitmentScheme> TrustedProgramCommitments<PCS> {
    ...
    pub fn derive(
        ...
        let context = CommitmentContext::UntrustedAdvice {
            len: program_image_num_words,
        };
        let coeff_layout = PCS::coefficient_layout(config, context);
        let commitment_layout = PCS::commitment_layout(config, context);
        let program_image_num_columns = coeff_layout.num_columns;
        let (program_image_commitment, program_image_hint) =
            pcs.commit(&commitment_layout, &program_image_poly, generators);
