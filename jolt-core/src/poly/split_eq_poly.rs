...
impl<F: JoltField> GruenSplitEqPolynomial<F> {
    ...
    pub fn dump(&self) -> std::collections::HashMap<String, Vec<F>> {
        ...
        // Dump current scalar
        data.insert(
            "gruen_split_eq.current_scalar".to_string(),
            vec![self.current_scalar],
        );

        // Dump current index
        data.insert(
            "gruen_split_eq.current_index".to_string(),
        ...
        if !self.E_in_vec.is_empty() {
            ...
            data.insert("gruen_split_eq.E_in_vec".to_string(), e_in_values);
        }
        ...
        if !self.E_out_vec.is_empty() {
            ...
            data.insert("gruen_split_eq.E_out_vec".to_string(), e_out_values);
        }
