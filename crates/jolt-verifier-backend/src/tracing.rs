...
mod tests {
    ...
    use super::*;
    use crate::expr_eval::evaluate_expr;
    ...
    /// `evaluate_expr` against `Tracing` records exactly one node per
    /// `Expr` arena entry, plus the wrapped openings/challenges. Replaying
    /// the resulting graph reproduces `Expr::evaluate`.
    #[test]
    fn tracing_then_replay_matches_expr_evaluate() {
        ...
        let expr = b.build(gamma * (h * h - h) + alpha);

        for _ in 0..8 {
            let h_v = Fr::random(&mut rng);
            let gamma_v = Fr::random(&mut rng);
            let alpha_v = Fr::random(&mut rng);

            let direct: Fr = expr.evaluate(&[h_v], &[gamma_v, alpha_v]);

            let mut tracer = Tracing::<Fr>::new();
            let h_w = tracer.wrap_proof(h_v, "h");
            let gamma_w = tracer.wrap_challenge(gamma_v, "gamma");
            let alpha_w = tracer.wrap_challenge(alpha_v, "alpha");
