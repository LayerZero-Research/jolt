...
const AKITA_WORKER_STACK_BYTES: usize = 64 * 1024 * 1024;

fn with_akita_pool<T: Send>(run: impl FnOnce() -> T + Send) -> T {
    static POOL: OnceLock<ThreadPool> = OnceLock::new();
    POOL.get_or_init(|| {
        ThreadPoolBuilder::new()
            .stack_size(AKITA_WORKER_STACK_BYTES)
            .build()
            .expect("Akita benchmark worker pool should build")
    })
    .install(run)
}
...
type DenseBackendHint = CommitmentHandle<AkitaField, AkitaField>;
type OneHotBackendHint = CommitmentHandle<AkitaField, AkitaField>;
...
struct AkitaProverBenchSetup {
    ...
    dense_backend: CpuBackend<AkitaField, AkitaField>,
    one_hot_scheme: OneHotBackendScheme,
    one_hot_prover: BackendSetup,
    one_hot_backend: CpuBackend<AkitaField, AkitaField>,
}
...
struct AkitaBatchCase {
    ...
    packing: PrefixPackedLayout<BatchId>,
    packed_polynomial: Polynomial<AkitaField>,
    packed_claims: PrefixPackedClaims<AkitaField>,
}
...
fn materialize_packed(
...
) -> (PrefixPackedLayout<BatchId>, Polynomial<AkitaField>) {
    let logical_num_vars = polynomials
        .first()
        .expect("packed batch is nonempty")
        .1
        .num_vars();
    assert!(polynomials
        .iter()
        .all(|(_, polynomial)| polynomial.num_vars() == logical_num_vars));
    let packing = PrefixPackedLayout::new(
        logical_num_vars,
        BATCH_POLYS,
        polynomials.iter().map(|(id, _)| *id),
    )
    .expect("valid fixed prefix packing");
    ...
    for (id, polynomial) in polynomials {
        for (local_index, evaluation) in polynomial.evals().iter().copied().enumerate() {
            let packed_index = packing
                .packed_index(id, local_index)
                .expect("logical index lies in the fixed prefix slot");
            packed_evaluations[packed_index] = evaluation;
        }
    }
...
fn akita_case(num_vars: usize) -> AkitaCase {
    ...
    let (backend_prover, backend, one_hot_backend_prover, one_hot_backend) =
        with_akita_pool(|| {
            let backend_prover = dense_scheme
                .setup_prover(num_vars, NUM_POLYS)
                .expect("Akita backend setup should succeed");
            let backend =
                CpuBackend::<AkitaField, AkitaField>::new(backend_prover.expanded.clone())
                    .expect("Akita backend initialization should succeed");
            let one_hot_backend_prover = one_hot_scheme
                .setup_prover(num_vars, NUM_POLYS)
                .expect("Akita one-hot backend setup should succeed");
            let one_hot_backend =
                CpuBackend::<AkitaField, AkitaField>::new(one_hot_backend_prover.expanded.clone())
                    .expect("Akita one-hot backend initialization should succeed");
            (
                backend_prover,
                backend,
                one_hot_backend_prover,
                one_hot_backend,
            )
        });
...
fn packed_batch_open(
    ...
    let physical = case
        ...
        .expect("fixed prefix claim should reduce");
    AkitaScheme::open(
        &case.packed_polynomial,
        physical.point.as_slice(),
        physical.value,
        &case.packed_pcs_setup,
        Some(hint),
        &mut transcript,
    )
...
fn bench_akita_packed_open(c: &mut Criterion) {
    ...
    for logical_num_vars in batch_logical_num_vars_cases() {
        ...
        configure_group(&mut group, physical_num_vars);
        let case = akita_batch_case(logical_num_vars);
        let (_, hint) = packed_batch_commit(&case);
        group.bench_function(BenchmarkId::from_parameter("packed_prove"), |b| {
            b.iter_batched(
                || hint.clone(),
                |hint| black_box(packed_batch_open(&case, hint)),
                BatchSize::SmallInput,
            );
        });
        group.finish();
