...
use jolt_verifier::stages::stage2::{
    ...
    Stage2BatchExpectedOutputClaims, Stage2BatchInputClaimRequest, Stage2BatchInputClaims,
    Stage2BatchOpeningPoints, Stage2BatchPointRequest, Stage2ClearOutputRequest,
    Stage2ExpectedOutputRequest, Stage2ProductUniSkipClearRequest,
    Stage2ProductUniSkipOutputClaimData, Stage2RegularBatchClearRequest,
};
    ...
    VC: VectorCommitment<Field = F>,
{
    ...
    let built_batch = batch.proof.finish(&batch_output_claim_values, transcript)?;
...
trait Stage2RegularBatchProofRecorder<F: Field> {
    type Proof;

    fn absorb_input_claims<T>(
        &mut self,
        ram_read_write_input_claim: F,
        instances: &[SumcheckRegularBatchInstance<F>],
        ram_raf_input_claim: F,
        transcript: &mut T,
    ) where
        T: Transcript<Challenge = F>;
    ...
    fn finish(self) -> Result<Self::Proof, ProverError>;
}

struct ClearStage2RegularBatchProofRecorder<F: Field, C> {
    round_polynomials: Vec<jolt_poly::CompressedPoly<F>>,
    _marker: std::marker::PhantomData<C>,
}

impl<F, C> ClearStage2RegularBatchProofRecorder<F, C>
where
    F: Field,
{
    fn new() -> Self {
...
impl<F, C> Stage2RegularBatchProofRecorder<F> for ClearStage2RegularBatchProofRecorder<F, C>
where
    F: Field,
{
    type Proof = SumcheckProof<F, C>;
    ...
    fn absorb_input_claims<T>(
        ...
        T: Transcript<Challenge = F>,
    {
        append_sumcheck_claim(transcript, &ram_read_write_input_claim);
        for instance in instances {
            append_sumcheck_claim(transcript, &instance.input_claim);
        }
        append_sumcheck_claim(transcript, &ram_raf_input_claim);
    ...
    fn finish(self) -> Result<Self::Proof, ProverError> {
        Ok(SumcheckProof::Clear(ClearProof::Compressed(
...
#[cfg(feature = "zk")]
struct CommittedStage2RegularBatchProofRecorder<'a, F, VC>
where
    F: Field,
    VC: VectorCommitment<Field = F>,
{
...
#[cfg(feature = "zk")]
impl<'a, F, VC> CommittedStage2RegularBatchProofRecorder<'a, F, VC>
where
    F: Field,
    VC: VectorCommitment<Field = F>,
{
...
#[cfg(feature = "zk")]
impl<'a, F, VC> Stage2RegularBatchProofRecorder<F>
    for CommittedStage2RegularBatchProofRecorder<'a, F, VC>
where
    F: Field,
    VC: VectorCommitment<Field = F>,
{
    type Proof = PendingCommittedRegularBatch<'a, F, VC>;

    fn absorb_input_claims<T>(
        &mut self,
        _ram_read_write_input_claim: F,
        _instances: &[SumcheckRegularBatchInstance<F>],
        _ram_raf_input_claim: F,
        _transcript: &mut T,
    ) where
        T: Transcript<Challenge = F>,
    {
...
fn prove_regular_batch_sumcheck_with_recorder<F, T, B, R>(
    ram_read_write: SumcheckRamReadWriteStateRequest<F>,
    ram_raf: SumcheckRamRafStateRequest<F>,
    ram_output_check: SumcheckRamOutputCheckStateRequest<F>,
    regular_batch: SumcheckStage2RegularBatchStateRequest<F>,
...
) -> Result<RegularBatchProof<F, R::Proof>, ProverError>
where
    F: Field,
    T: Transcript<Challenge = F>,
    B: SumcheckBackend<F, JoltVmNamespace>
    ...
    R: Stage2RegularBatchProofRecorder<F>,
{
    ...
    let proof = proof_recorder.finish()?;
...
fn prove_regular_batch_sumcheck_committed<'a, F, T, B, VC>(
    ...
    vc_setup: &'a VC::Setup,
    transcript: &mut T,
) -> Result<RegularBatchProof<F, PendingCommittedRegularBatch<'a, F, VC>>, ProverError>
where
    F: Field,
    T: Transcript<Challenge = F>,
    B: SumcheckBackend<F, JoltVmNamespace>
