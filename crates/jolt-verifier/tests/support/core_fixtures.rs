...
fn generate_committed_muldiv() -> GeneratedCoreFixture {
    ...
    let full = jolt_core::zkvm::program::FullProgramPreprocessing::preprocess(
        bytecode,
        init_memory_state,
        entry_address,
    )
    .expect("preprocess committed program image");
    let prover_preprocessing = JoltProverPreprocessing::preprocess_committed(
...
fn convert_preprocessing(
    ...
    let program = jolt_verifier::ProgramPreprocessing::Full(JoltProgramPreprocessing {
